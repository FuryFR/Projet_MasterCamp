...
DB_PATH = '/home/issey/Desktop/mastercamp/users.db'
KEYS_DIR = '/home/issey/Desktop/mastercamp/keys.pem'
...
def register_user(username, password, email):
    try:
        ...
        # Example database insertion (using SQLite3)
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        try:
            c.execute('INSERT INTO users (username, password, email) VALUES (?, ?, ?)', (username, hashed_password, email))
            conn.commit()
            # Optionally, generate keys for the newly registered user
...
def handle_initial_connection(client_socket, addr):
    try:
        ...
        try:
            ...
            client_socket.send(base64.urlsafe_b64encode(fernet.encrypt("Invalid Fernet token".encode('utf-8'))))
            client_socket.close()
            return
        except Exception as e:
            client_socket.send(base64.urlsafe_b64encode(fernet.encrypt(f"Decryption error: {str(e)}".encode('utf-8'))))
            client_socket.close()
            return
        
        if decrypted_message.startswith("LOGIN"):
            _, username, password = decrypted_message.split()
            if authenticate_user(username, password):
                client_socket.send(base64.urlsafe_b64encode(fernet.encrypt("OTP required".encode('utf-8'))))
                otp_message = client_socket.recv(1024)
                try:
                    otp_message = fernet.decrypt(base64.urlsafe_b64decode(otp_message)).decode('utf-8')
                except InvalidToken:
                    client_socket.send(base64.urlsafe_b64encode(fernet.encrypt("Invalid Fernet token for OTP".encode('utf-8'))))
                    client_socket.close()
                    return
                if otp_message.startswith("OTP"):
                    _, otp = otp_message.split()
                    if verify_otp(username, otp):
                        client_socket.send(base64.urlsafe_b64encode(fernet.encrypt("Login successful".encode('utf-8'))))
                        handle_room_selection(client_socket, addr, username)
                    else:
                        client_socket.send(base64.urlsafe_b64encode(fernet.encrypt("OTP verification failed".encode('utf-8'))))
                        client_socket.close()
                else:
                    client_socket.send(base64.urlsafe_b64encode(fernet.encrypt("OTP verification required".encode('utf-8'))))
                    client_socket.close()
            else:
                client_socket.send(base64.urlsafe_b64encode(fernet.encrypt("Login failed".encode('utf-8'))))
                client_socket.close()
        elif decrypted_message.startswith("REGISTER"):
            _, username, password, email = decrypted_message.split()
            response = register_user(username, password, email)
            client_socket.send(base64.urlsafe_b64encode(fernet.encrypt(response.encode('utf-8'))))
        else:
            client_socket.send(base64.urlsafe_b64encode(fernet.encrypt("Invalid command".encode('utf-8'))))
            client_socket.close()
    except Exception as e:
        print(f"Error: {str(e)}")
        client_socket.send(base64.urlsafe_b64encode(fernet.encrypt(f"Server error: {str(e)}".encode('utf-8'))))
        client_socket.close()
...
def main():
    ...
    server.bind(("192.168.0.21", 9999))  # Écoute sur toutes les interfaces réseau
    server.listen(5)
