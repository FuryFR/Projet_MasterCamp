...
def connect():
    ...
    try:
        ...
        server_ip = "192.168.190.139"  # Remplacez par l'adresse IP de votre serveur
...
def send_message(chat_text, input_text):
    ...
    if message and client_socket:
        try:
            # Charger la clé publique du destinataire (par exemple, client2)
            public_key_path = "/Public/rsa_keys/client2_public_key.pem"
            if os.path.exists(public_key_path):
                public_key = load_public_key(public_key_path)
                encrypted_message = encrypt_message(public_key, message)

                # Créer un en-tête indiquant la longueur du message
                message_length = len(encrypted_message)
                header = message_length.to_bytes(4, byteorder='big')
                full_message = header + encrypted_message

                # Impressions de débogage
                print(f"Message original : {message}")
                print(f"Message chiffré : {encrypted_message}")
                print(f"Longueur du message chiffré envoyé : {len(encrypted_message)}")

                # Envoyer le message avec l'en-tête
                client_socket.send(full_message)

                # Afficher le message dans la chatbox
...
def receive_messages():
    # Charger la clé privée du client pour déchiffrer les messages
    private_key_path = "/Public/rsa_keys/client1_private_key.pem"  # Mettez le chemin correct vers votre clé privée
    if os.path.exists(private_key_path):
        private_key = load_private_key(private_key_path)
    else:
        print("Erreur : fichier de clé privée introuvable.")
        return

    while True:
        try:
            # Lire l'en-tête pour obtenir la longueur des informations sur l'envoyeur
            ...
            # Lire les informations sur l'envoyeur
            ...
            # Impressions de débogage
            print(f"Longueur du message chiffré reçu : {len(encrypted_message)}")
            ...
            try:
                ...
                print(f"Message déchiffré : {message}")  # Impressions de débogage
            except Exception as e:
                print(f"Erreur de déchiffrement : {str(e)}")
                continue

            # Afficher le message déchiffré dans la chatbox
            chat_text.config(state=tk.NORMAL)
